...
test-bash3: # Hardcoding Bash v3.2.57 specifically because that's the version that Macs are stuck on.
	@docker run --volume="$(CURDIR):/tmp" --workdir="/tmp" bash:3.2.57 ./test.sh
	@echo

test-bash4:
	@docker run --volume="$(CURDIR):/tmp" --workdir="/tmp" bash:4 ./test.sh
	@echo

test-bash5:
	@docker run --volume="$(CURDIR):/tmp" --workdir="/tmp" bash:5 ./test.sh
	@echo
