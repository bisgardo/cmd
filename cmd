...
function __cmd_mv {
  ...
  local PS3='Root: ' cmd_root_new="$cmd_root" path_from_root_new
  select cmd_root_new in "${roots[@]}"; do
    if [ "$cmd_root_new" ]; then break; fi
  done
  path_from_root_new="${1:-$(cmd_ask 'New path' "$path_from_root")}"
  ...
  # the move would otherwise produce an ambiguous command (or overwrite the file).
  local run_scripts_existing count=0
  run_scripts_existing="$(
    for r in "${roots[@]}"; do
      if [ "$r" != "$cmd_root" ] || [ "$path_from_root_new" != "$path_from_root" ]; then
...
function __cmd_mv_file {
  # caller: __cmd_mv
  # scope: cmd_file_old, cmd_file, ...
  mkdir -p "$(dirname "$cmd_file")" # $cmd_dir is the old file's dir
